...
import com.midgar.hotel_midgar.modelo.util.CategoriaTableModel;
...
public class CategoriaController implements ActionListener, MouseListener {
    ...
    public CategoriaController(Dashboard dashboard) {
        ...
        inicializarComboBoxCategoriaNaSuite();
    }
    
    private void inicializarComboBoxCategoriaNaSuite() {
        this.dashboard.getComboBoxSuiteCategoria().removeAllItems();
        this.dashboard.getComboBoxSuiteCategoria().addItem("Selecione a categoria");
        categorias.stream().forEach(c -> {
            this.dashboard.getComboBoxSuiteCategoria().addItem(c.getNome());
        });
    }    
    ...
    private void validacaoDoCampo(String campo) {
        if(campo.isEmpty()) {
            String mensagem = "Preencher o campo nome";
            mensagemNaTela(mensagem, Color.RED);
    ...
    private void apagar() {
        if (categoria != null) {
            ...
            mensagemNaTela("Selecionar uma categoria na tabela", Color.RED);
        }
    }
